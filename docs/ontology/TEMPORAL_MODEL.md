...
An `ApplicabilityAssessment`'s `data_version` field (already established in FOUNDATION.md §4b) captures a marker of the *entire* aircraft configuration snapshot used — the as-of state of every relevant `EngineInstallation`/`ComponentInstallation` row at `evaluated_at`. This is what makes an old assessment's reasoning reproducible without re-querying live installation history that may have since changed: the assessment doesn't store the snapshot, but it stores enough to know (and, if needed, regenerate) exactly which snapshot was used.
...
The CTO's review specifically asked how AeroComply avoids a historical assessment accidentally using today's configuration. Two independent safeguards, not one:
...
2. **`data_version` is computed once, at evaluation time, and stored on the `ApplicabilityAssessment` row.** Even if (2) were somehow bypassed and a future re-run of "the same" historical assessment queried live data instead of the original as-of snapshot, the *stored* `data_version` from the original assessment would not match a freshly recomputed one — making any such regression immediately detectable (a reproducibility check: re-evaluating an old assessment's rule/document/data version triple should always reproduce its stored `data_version`, and a mismatch is a bug signal, not a silent data change). This mirrors exactly the reproducibility guarantee ADR-004/FOUNDATION.md §12 already established for rule versioning, applied here specifically to the *data* side of that triple.

Together, these mean "today's configuration leaking into a historical assessment" would require *both* an explicit-timestamp-parameter violation *and* a silent `data_version` mismatch going unnoticed — a two-layer defense rather than a single trust point.
