...
**Real limitation surfaced, not fixed (out of scope for this pass, logged to `docs/BUILD_BACKLOG.md`):** there is no way to move a `Task.execution_state` to `COMPLETED` anywhere in the product today — no REST endpoint (`backend/app/api/v1/work_orders.py` only exposes create/list for tasks, no update/patch) and no UI control. This means a work order can never reach `READY` release-readiness in REAL mode purely through the app; the TASK_EXECUTION blocker is permanent until a task-completion capability is added. Separately, `RealTaskGatePanel`'s "Complete (as me)" button for a required (RII) inspection does not pass `inspector_user_id`, so it always fails backend validation ("An inspector_user_id is required...") for RII items — a pre-existing gap from the Inspections phase, not touched here since inspections files are out of scope for this phase.
