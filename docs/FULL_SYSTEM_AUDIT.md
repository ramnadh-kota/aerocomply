...
| evidence (per-task panel on `maintenance/work-orders/[id]` via `RealTaskGatePanel`) | `useDataMode`/`isReal` present; `frontend/lib/api/evidence.ts` wired to `/evidence` create/get/transition; re-confirmed 2026-09-12 by browser click-through (create → UPLOADED → SUBMITTED) plus direct API verification of the remaining transitions (→ AWAITING_REVIEW → REJECTED, REJECTED→ACCEPTED correctly refused) through the same service path | REAL/PARTIAL REAL — the standalone `app/(app)/evidence` list/detail pages remain DEMO-only by design: they model a different, assessment-linked document concept with no backend equivalent (the backend `Evidence` model is a per-task lifecycle/status record with no file field), not an unwired gap |
| compliance (`compliance/page.tsx`, `compliance/regulatory-register/page.tsx`) | `useDataMode`/`isReal` present (added 2026-09-12) — new `frontend/lib/api/compliance.ts` wired to `/regulatory-requirements` and `/aircraft/{id}/compliance-assessments`; browser-verified REAL mode against a seeded org (2 requirements, 2 assessments) and DEMO mode confirmed unaffected | REAL/PARTIAL REAL — see §7/§13: both pages carry an explicit in-UI notice ("Manually recorded, not automated") since `ComplianceAssessment` rows are human-entered determinations, never the output of the (admittedly absent) applicability engine. `compliance/pre-audit` remains MOCK-only — out of scope for this pass, it composes evidence/inspections/parts/findings data owned by other in-flight work |
| release-readiness (new `RealReleaseReadinessPanel` on `maintenance/work-orders/[id]`) | `useDataMode`/`isReal` present (added 2026-09-12) — new `frontend/lib/api/release-readiness.ts` wired to `GET /work-orders/{id}/release-readiness`; browser-verified live against a seeded org — panel showed `BLOCKED` with a `TASK_EXECUTION` blocker for a freshly created task, then live-updated (via its own Refresh button, and again on a full navigate-away/navigate-back) to reflect newly created EVIDENCE/INSPECTION blockers and their later resolution (evidence ACCEPTED removed the EVIDENCE blocker) with no stale data at any step | REAL/PARTIAL REAL — presentation-only over the backend's existing three blocker categories (EVIDENCE, INSPECTION, TASK_EXECUTION); the standalone fleet-wide `maintenance/release-readiness` page and the `maintenance/planning/[id]` inline readiness section remain DEMO-only (`getReleaseReadinessForWorkOrder` from `lib/mock/ai/analytics.ts`), untouched by this pass, and continue to model additional MATERIAL/QUALIFICATION/DEFERRED/REGULATORY/AUDIT categories the backend does not yet implement |
| regulations, dashboard, executive, finance, fleet, pilot, workspace, organization, platform, reports, documents, automation, integrations, notifications, engines, audit, data-import | no `useDataMode`/`isReal` hit found | MOCK or STATIC (UNCONFIRMED per-page; inferred from absence of the real/mock switch pattern and the repo-wide 78-file `lib/mock` import count) |
...
| Release Readiness | (computed, not a standalone tenant table — reads from work_order/task/inspection/evidence) | N/A (derived view) | `release_readiness_service` — inherits scoping from the entities it reads | UNKNOWN | n/a | n/a | n/a | inherits from parents | delegates | test_release_readiness_service.py (unit) | PARTIAL — frontend now reads this endpoint (2026-09-12), tenancy behavior itself unchanged/unretested this pass |
...
## 23. REAL-Mode Frontend Rollout — 5-Phase Rollup (completed 2026-09-12)

Five phases wired REAL-mode frontend coverage for the domains flagged UNKNOWN/PARTIAL in earlier passes of this audit, each following the existing `useDataMode`/`isReal`/`RealDataPanel` pattern with no client-side reimplementation of backend gate logic:

1. **Evidence** — found already wired from an earlier commit (`3dd86fa`); no new work needed.
2. **Inspections/RII** (`771869d`) — `maintenance/inspections` list + `[id]` detail, `frontend/lib/api/inspections.ts`.
3. **Deferred/MEL** (`81b2c41`) — `maintenance/deferred`, `frontend/lib/api/deferred-items.ts`.
4. **Compliance** (`d322705`) — `compliance/page.tsx`, `compliance/regulatory-register/page.tsx`, `frontend/lib/api/compliance.ts`.
5. **Release Readiness** (this pass) — `RealReleaseReadinessPanel` on `maintenance/work-orders/[id]`, `frontend/lib/api/release-readiness.ts`.

**Live cross-workflow trace (this pass):** registered a fresh org, created an aircraft, a work order, and a task via the real API, then drove Evidence (create → SUBMITTED → AWAITING_REVIEW → ACCEPTED) and Inspection (create RII) through the browser UI on the work-order detail page, and confirmed `RealReleaseReadinessPanel` moved from 1 blocker (TASK_EXECUTION only) → 3 blockers (+ EVIDENCE, + INSPECTION) → 2 blockers (EVIDENCE cleared after Accept) — reflected correctly both via the panel's own Refresh button and after a full navigate-away/navigate-back, i.e. no stale-cache bug found in this trace.

**Real limitation surfaced, then fixed 2026-09-13 (BL-16/BL-17, see `docs/BUILD_BACKLOG.md`):** at the time of the above trace there was no way to move a `Task.execution_state` to `COMPLETED` anywhere in the product, so a work order could never reach `READY` release-readiness in REAL mode purely through the app, and `RealTaskGatePanel`'s "Complete (as me)" button for a required (RII) inspection always failed backend validation. Both are now closed: `POST /work-orders/{work_order_id}/tasks/{task_id}/complete` (`work_order_service.complete_task`) plus a "Complete Task" button on `maintenance/work-orders/[id]` clears the TASK_EXECUTION blocker; `inspection_service.transition_inspection_requirement` now derives the inspector identity server-side from the authenticated actor when the request omits `inspector_user_id` (with an explicit override still available for a supervisor completing on behalf of a named inspector, now validated against the caller's own organization). Live-browser re-trace 2026-09-13 on a fresh work order confirmed BLOCKED (TASK_EXECUTION) → Complete Task → READY (0 blockers), and a separately created RII inspection completed successfully via "Complete (as me)" with no error. Backend integration coverage: `test_task_completion_api.py` (5 tests), `test_inspection_completion_identity_api.py` (4 tests), `test_release_readiness_full_chain.py` (1 end-to-end test tracing task → evidence → inspection → READY). Full backend suite: 366/366 passing.

**Known remaining limitations across all 5 phases (see individual phase notes in §11 above for detail):** `compliance/pre-audit` and the fleet-wide `maintenance/release-readiness` + `maintenance/planning/[id]` pages remain DEMO-only by design/scope; the regulatory applicability condition-tree engine is still absent; per-model tenancy audit for several domains (technicians, AOG, maintenance, inventory, parts, vendors, compliance) remains PARTIAL/UNKNOWN per §22; Lisa live frontend↔backend wiring for the `/ai` page was not separately re-verified this pass (Lisa's test suite — 64 tests — was re-run and confirmed passing with no regression from any of the 5 phases' work).

**Next dependency-safe engineering sequence:** (1) add a task-completion/update endpoint + UI control, the one missing piece blocking any work order from reaching real `READY` status; (2) fix `RealTaskGatePanel`'s RII completion to collect/pass `inspector_user_id`; (3) decide and build (or explicitly defer) the regulatory applicability engine, since compliance and release-readiness both currently route around it; (4) backfill API-level integration tests for work_orders/evidence/inspections/deferred_items per §19; (5) complete the per-model tenancy audit in §22 before any production launch.
