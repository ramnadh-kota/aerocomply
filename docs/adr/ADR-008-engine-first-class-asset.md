...
**Note (CTO review, post-v1.0)**: `ComponentInstallation` has the identical polymorphism problem (its parent asset is either an `Aircraft` or an `Engine`) and was originally left with a *weaker*, untyped `parent_asset_id`/`parent_asset_type` pair rather than this pattern — an inconsistency the CTO's review caught. `ComponentInstallation` has been corrected to use the same dual-typed-column approach described below (`aircraft_parent_id`/`engine_parent_id` + `parent_asset_type` + `CHECK` constraint); see [AVIATION_ONTOLOGY.md §4](../ontology/AVIATION_ONTOLOGY.md). The pattern documented here is now the standard AeroComply uses for *every* polymorphic-subject case, not just this one.
